...
class RomanNumeralConverterTest(unittest.TestCase):
    ...
    def all(self):
        v=unittest.TestLoader().loadTestsFromTestCase(RomanNumeralConverterTest)
        unittest.TextTestRunner(verbosity=2).run(v)
    

if __name__ == "__main__":
    v=["high_and_low", "combos", "all"]
    
    for suite_func in v:
        suite = unittest.TestSuite()
        print( "Corriendo el test: '%s'" % suite_func)
        t=RomanNumeralConverterTest(suite_func)
        suite=t
